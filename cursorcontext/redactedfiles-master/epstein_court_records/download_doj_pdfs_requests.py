...
def main():
    ...
    if download_file(hub_url, hub_file, session):
        print("✓ Downloaded hub.html")
    else:
        print("✗ Failed to download hub page", file=sys.stderr)
        print("\nTip: The DOJ website may be blocking automated requests.", file=sys.stderr)
        print("Try manually downloading the page and saving as 'hub.html', then run again.", file=sys.stderr)
        return 1
    ...
    for i, url in enumerate(sorted(pages), 1):
        ...
        if download_file(url, page_file, session):
    ...
    if shutil.which("aria2c"):
        ...
        for i, url in enumerate(sorted(pdfs), 1):
            ...
            if download_file(url, out_file, session):
                downloaded += 1
            else:
                failed += 1
