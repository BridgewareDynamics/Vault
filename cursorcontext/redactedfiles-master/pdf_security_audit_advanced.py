                    ...
                    for line in block.get("lines", []):
                        for span in line.get("spans", []):
                            ...
                            if color > 16710886:  # ~99% white
                                if page_num not in white_on_white_pages:
                            ...
                            if bbox[0] < page_rect.x0 - 100 or bbox[1] < page_rect.y0 - 100 or \
                               bbox[2] > page_rect.x1 + 100 or bbox[3] > page_rect.y1 + 100:
                                if page_num not in offpage_pages:
...
def format_advanced_report(findings: AdvancedSecurityFindings, verbose: bool = True) -> str:
    """Format advanced security findings as detailed text report."""
    lines = []
    lines.append("=" * 80)
    lines.append("ADVANCED PDF SECURITY & PRIVACY AUDIT")
    lines.append("=" * 80)
    lines.append(f"\n🔒 OVERALL RISK SCORE: {findings.risk_score}/100")
    
    if findings.risk_score >= 70:
        lines.append("   ⚠️  HIGH RISK - Multiple serious privacy/security issues detected")
    elif findings.risk_score >= 40:
        lines.append("   ⚠️  MEDIUM RISK - Several privacy concerns found")
    else:
        lines.append("   ✓ LOW RISK - Minor issues only")
    
    lines.append("\n" + "=" * 80)
    lines.append("1. METADATA LEAKAGE")
    lines.append("=" * 80)
    if findings.has_metadata:
        lines.append(f"⚠️  Metadata Present: {len(findings.metadata_keys)} field(s)")
        if verbose and findings.metadata_details:
            for key, value in findings.metadata_details.items():
                lines.append(f"   • {key}: {value}")
    else:
        lines.append("✓ No metadata detected")
    
    lines.append("\n" + "=" * 80)
    lines.append("2. HIDDEN/INVISIBLE TEXT")
    lines.append("=" * 80)
    if findings.has_hidden_text:
        lines.append(f"⚠️  Hidden Text Detected: {', '.join(findings.hidden_text_types)}")
        if findings.white_on_white_pages:
            lines.append(f"   • White-on-white text: pages {findings.white_on_white_pages}")
        if findings.offpage_text_pages:
            lines.append(f"   • Off-page text: pages {findings.offpage_text_pages}")
    else:
        lines.append("✓ No hidden text detected")
    
    lines.append("\n" + "=" * 80)
    lines.append("3. REDACTION ISSUES")
    lines.append("=" * 80)
    if findings.overlay_risk in ["YES", "MAYBE"]:
        lines.append(f"⚠️  Overlay Redaction Risk: {findings.overlay_risk}")
        lines.append("   ⚠️  BLACK BOXES MAY BE REMOVABLE - TEXT STILL UNDERNEATH!")
    else:
        lines.append("✓ No overlay redaction detected")
    
    lines.append("\n" + "=" * 80)
    lines.append("4. EMBEDDED FILES & ATTACHMENTS")
    lines.append("=" * 80)
    if findings.has_attachments:
        lines.append(f"⚠️  Embedded Attachments: {findings.attachment_count} file(s)")
        if verbose and findings.attachment_names:
            for name in findings.attachment_names:
                lines.append(f"   • {name}")
    else:
        lines.append("✓ No attachments")
    
    lines.append("\n" + "=" * 80)
    lines.append("5. FORMS & COMMENTS")
    lines.append("=" * 80)
    if findings.has_annotations:
        lines.append(f"⚠️  Annotations: {findings.annotation_count}")
        if verbose and findings.annotation_types:
            lines.append(f"   Types: {', '.join(findings.annotation_types)}")
    else:
        lines.append("✓ No annotations")
    
    if findings.has_forms:
        lines.append(f"⚠️  Form Fields: {findings.form_field_count}")
    else:
        lines.append("✓ No forms")
    
    if findings.reviewer_names:
        lines.append(f"⚠️  Reviewer Names Found: {', '.join(findings.reviewer_names)}")
    
    lines.append("\n" + "=" * 80)
    lines.append("6. OCR & SCANNED DOCUMENTS")
    lines.append("=" * 80)
    if findings.is_scanned:
        lines.append("⚠️  Document appears to be scanned")
    if findings.has_ocr_layer:
        lines.append(f"⚠️  OCR Text Layer Detected: {len(findings.ocr_pages)} page(s)")
        lines.append("   Hidden searchable text may exist behind images")
    else:
        lines.append("✓ No OCR layer detected")
    
    lines.append("\n" + "=" * 80)
    lines.append("7. JAVASCRIPT")
    lines.append("=" * 80)
    if findings.has_javascript:
        lines.append(f"⚠️  JavaScript Present (Potential Security Risk)")
        lines.append("   Could execute code, track usage, or phone home")
    else:
        lines.append("✓ No JavaScript")
    
    lines.append("\n" + "=" * 80)
    lines.append("8. EXTERNAL RESOURCES & TRACKING")
    lines.append("=" * 80)
    if findings.has_external_links:
        lines.append(f"⚠️  External Links: {len(findings.external_urls)} URL(s)")
        if verbose and findings.external_urls and len(findings.external_urls) <= 10:
            for url in findings.external_urls:
                lines.append(f"   • {url}")
        elif len(findings.external_urls) > 10:
            lines.append(f"   (Showing first 10 of {len(findings.external_urls)})")
            for url in findings.external_urls[:10]:
                lines.append(f"   • {url}")
    else:
        lines.append("✓ No external links")
    
    if findings.has_remote_resources:
        lines.append("⚠️  Remote Resources Referenced (May reveal IP/location when opened)")
    
    lines.append("\n" + "=" * 80)
    lines.append("9. DIGITAL SIGNATURES")
    lines.append("=" * 80)
    if findings.has_signatures:
        lines.append(f"⚠️  Digital Signatures: {findings.signature_count}")
        if findings.signer_names:
            lines.append(f"   Signers: {', '.join(findings.signer_names)}")
        lines.append("   May expose full legal name, email, organization")
    else:
        lines.append("✓ No signatures")
    
    lines.append("\n" + "=" * 80)
    lines.append("10. VERSION HISTORY & UPDATES")
    lines.append("=" * 80)
    if findings.incremental_updates:
        lines.append(f"⚠️  Incremental Updates Detected (~{findings.version_count} versions)")
        lines.append("   May contain earlier drafts or deleted content")
    else:
        lines.append("✓ No version history")
    
    if findings.has_layers:
        lines.append(f"⚠️  Optional Content Groups (Layers): {findings.layer_count}")
        lines.append("   Hidden layers may contain sensitive content")
    
    lines.append("\n" + "=" * 80)
    lines.append("11. ACCESSIBILITY DATA")
    lines.append("=" * 80)
    if findings.has_structure_tags:
        lines.append("⚠️  Structure Tags Present")
    if findings.has_alt_text:
        lines.append("⚠️  Alt Text Present (may describe redacted content)")
    if not findings.has_structure_tags and not findings.has_alt_text:
        lines.append("✓ No accessibility metadata")
    
    lines.append("\n" + "=" * 80)
    lines.append("12. WATERMARKS & OTHER")
    lines.append("=" * 80)
    if findings.has_watermarks:
        lines.append("⚠️  Potential Watermarks Detected")
        lines.append("   May be used for leak tracing")
    else:
        lines.append("✓ No obvious watermarks")
    
    if findings.has_actions:
        lines.append("⚠️  Document Actions (auto-execute on open)")
    
    if findings.has_thumbnails:
        lines.append("⚠️  Thumbnail Images (preview of potentially sensitive pages)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python pdf_security_audit_advanced.py <pdf_file>")
        sys.exit(1)
    ...
    report = format_advanced_report(findings, verbose=True)
