...
def audit_pdf_security(pdf_path: Path) -> SecurityFindings:
    ...
    try:
        import sys
        print("PROGRESS:Checking PDF metadata...", file=sys.stderr, flush=True)
        ...
        print("PROGRESS:Checking for embedded attachments...", file=sys.stderr, flush=True)
        
        # Attachments check
        findings.has_attachments, findings.attachment_count = check_attachments(doc)
        print("PROGRESS:Scanning for annotations and comments...", file=sys.stderr, flush=True)
        
        # Annotations check
        findings.has_annotations, findings.annotation_count = check_annotations(doc)
        print("PROGRESS:Checking for form fields...", file=sys.stderr, flush=True)
        ...
        if HAS_PIKEPDF:
            print("PROGRESS:Running advanced security checks (layers, JavaScript, actions)...", file=sys.stderr, flush=True)
        ...
        print("PROGRESS:Checking for incremental updates...", file=sys.stderr, flush=True)
