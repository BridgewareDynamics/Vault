...
def audit_pdf(
    ...
    try:
        import sys
        print("PROGRESS:Opening PDF file...", file=sys.stderr, flush=True)
        doc = fitz.open(str(pdf_path))
        total_pages = len(doc)
        print(f"PROGRESS:PDF opened. Analyzing {total_pages} page(s)...", file=sys.stderr, flush=True)
        ...
        print("PROGRESS:Checking for overlay redactions...", file=sys.stderr, flush=True)
        for page_idx in range(total_pages):
            page = doc[page_idx]
            page_num = page_idx + 1  # 1-indexed for user display
            
            if page_idx % 10 == 0 or page_idx == total_pages - 1:
                print(f"PROGRESS:Analyzing page {page_num} of {total_pages}...", file=sys.stderr, flush=True)
        ...
        print(f"PROGRESS:Overlay analysis complete. Found {len(flagged_pages)} flagged page(s).", file=sys.stderr, flush=True)
        doc.close()
